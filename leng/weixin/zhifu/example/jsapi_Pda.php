...
<body>
	<form class="abc" action="../../../weixinnew/last.php" method="post" style="display:none;" id="aaa">
		收款成功：<input class="abcd" type="text" name="username"  value="PDA收款成功"/><br /> 
		订单号：<input type="text" name="password" class="Waybill"/><br />
		司机工号：<input type="text" name="driver" class="driver"/><br />
		...
		openID：<input class="_openID" type="text" name="123" value="oTarnvxZ-objNsHTLNVIYBV5rOdA" /><br /> 
			...
			WeixinJSBridge.invoke(
				...
				function(res) {
					...
					if(res.err_msg == "get_brand_wcpay_request:ok") {
						...
						var _payCity=window.sessionStorage.getItem("_city");
						var _driverId=window.sessionStorage.getItem("_driverId");
						var _Waybill=window.sessionStorage.getItem("_Waybill");
						...
						$(".pay_danwei").val(_daWei);//付款单位
						$(".time").val("服务城市："+_payCity);	//服务城市
						$(".driver").val("司机工号："+_driverId);	
						$(".Waybill").val(_Waybill+"(运单号)");				
						$.ajax({
							url: "http://www.ccsc58.com/json/weixin/01_00_weixin_zhifu_PDA_post.php",
							type:"post",
							data: {
								controller: "register",
							...
							success: function(data) {
								...
								if(_json.code == 10000 && _json.message == "success" && _json.resultCode == "success") {
									var _openId=["oTarnv5aWyxLcCENYrs5UOR3FqvQ"];
									for(var i=0;i<_openId.length;i++){
										...
										$.ajax({
											...
											error:function(){
												alert("网络错误，如已付款成功请联系'中集冷云客服人员'");
											}
										});
									}
									alert("中集冷云PDA平台提醒您：付款￥"+_money+"成功");
									window.location.href="../../Pda_pay.php";	
								} else {
